...
if [ -f "/root/mkfs.sh" ];then
  #The reboot command will be executed
  echo "@reboot /bin/bash /root/mkfs.sh >> $SOCA_HOST_SYSTEM_LOG/mkfs.log 2>&1" | crontab -
fi
...
if [[ "$SOCA_FSX_LUSTRE_BUCKET" != 'false' ]] || [[ "$SOCA_FSX_LUSTRE_DNS" != 'false' ]] ; then
    ...
    echo "export FSX_MOUNTPOINT=$FSX_MOUNTPOINT" >> /etc/environment
