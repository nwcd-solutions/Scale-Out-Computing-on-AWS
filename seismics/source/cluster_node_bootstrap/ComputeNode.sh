...
yum install -y https://nwcd-solutions.s3.cn-northwest-1.amazonaws.com.cn/scale-out-computing-on-aws/v2.6.0/amazon-ssm-agent.rpm
...
then
	...
	then
	    ...
	    then
            ...
            echo "#!/bin/bash" > /root/mkfs.sh
            echo "/sbin/mkfs -t ext4 $VOLUME_LIST" >> /root/mkfs.sh
            echo "mount $VOLUME_LIST /scratch" >> /root/mkfs.sh
            echo "chmod -R 777 /scratch" >> /root/mkfs.sh
            chmod 755 /root/mkfs.sh
...
# Add users
useradd -d /data/home/$DCV_USERNAME $DCV_USERNAME -M -s $DCV_USERSHELL
chmod go-w /data/home/$DCV_USERNAME
chown $DCV_USERNAME:$DCV_USERNAME -R /data/home/$DCV_USERNAME
echo "$DCV_USERNAME ALL=(ALL) NOPASSWD:ALL" >> /etc/sudoers
