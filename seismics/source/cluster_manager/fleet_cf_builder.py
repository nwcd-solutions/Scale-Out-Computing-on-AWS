...
echo export "DCV_USERNAME="''' + str(params['AdminUserName'])+ '''"" >> /etc/environment
echo export "DCV_USERSHELL="''' + str(params['AdminUserShell'])+ '''"" >> /etc/environment
...
# Mount EFS
echo "''' + params['EFSDataDns'] + ''':/data /data nfs4 nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport 0 0" >> /etc/fstab
echo "''' + params['EFSAppsDns'] + ''':/apps /apps nfs4 nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2,noresvport 0 0" >> /etc/fstab
EFS_MOUNT=0
mount -a 
...
/bin/bash /apps/soca/$SOCA_CONFIGURATION/cluster_node_bootstrap/ComputeNode.sh ''' + params['SchedulerHostname'] + ''' >> $SOCA_HOST_SYSTEM_LOG/ComputeNode.sh.log 2>&1'''
        ...
        # Begin EC2 Fleet Resource
        f=EC2Fleet("LaunchEC2Fleet")
        f.DependsOn="NodeLaunchTemplate"
        f.LaunchTemplateConfigs=[
            FleetLaunchTemplateConfigRequest(
        ...
        f.TargetCapacitySpecification=TargetCapacitySpecificationRequest(
